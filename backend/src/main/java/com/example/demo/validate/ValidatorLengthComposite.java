...
public class ValidatorLengthComposite {
  ...
  public HashMap<String, ErrorValidate> validate(Login client) {
    if (client.getUser()!=null) {
      addValidate(Validator.size.getName(), client.getUser(), 7, 7);
      addValidate(Validator.size.getName(), client.getPassword(), 8, 15);
    }
    return generateError();
  }
